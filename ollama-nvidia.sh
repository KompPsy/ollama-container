...
usage() {
  ...
  echo "  rebuild             - Rebuild the Ollama container"
...
case "$1" in
  ...
  rebuild)
    ...
    docker compose -f "$DOCKER_COMPOSE_FILE" build --no-cache ollama
    docker compose -f "$DOCKER_COMPOSE_FILE"  up -d --force-recreate ollama
    if [ $? -eq 0 ]; then
      echo "Ollama container started successfully."
    else
      echo "Error: Failed to start Ollama container."
